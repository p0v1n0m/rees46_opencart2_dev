...
class ControllerModuleRees46 extends Controller {
	...
	public function exportStatus($order) {
		if ($this->config->get('rees46_tracking_status')) {
			if ($this->config->get('rees46_status_created') && in_array($order['order_status_id'], $this->config->get('rees46_status_created'))) {
				$status = 0;
			} elseif ($this->config->get('rees46_status_completed') && in_array($order['order_status_id'], $this->config->get('rees46_status_completed'))) {
				$status = 1;
			} elseif ($this->config->get('rees46_status_cancelled') && in_array($order['order_status_id'], $this->config->get('rees46_status_cancelled'))) {
				$status = 2;
			}
			...
			if (isset($status)) {
				...
				if ($return['info']['http_code'] < 200 || $return['info']['http_code'] >= 300) {
					if ($this->config->get('rees46_log')) {
						$this->log->write('REES46 log: error autoexport status of order_id = ' . $order['order_id'] . ' [' . $return['info']['http_code'] . ']');
					}
				} else {
					if ($this->config->get('rees46_log')) {
						$this->log->write('REES46 log: success autoexport status of order_id = ' . $order['order_id']);
					}
				}
			}
		}
